...
describe('Chord', () => {
  describe('factory methods', () => {
    ...
    it('creates diminished and augmented triads', () => {
      expect(Chord.diminished('C').quality).toBe('diminished');
      expect(Chord.diminished('C').intervals).toEqual([0, 3, 6]);
      expect(Chord.augmented('C').quality).toBe('augmented');
      expect(Chord.augmented('C').intervals).toEqual([0, 4, 8]);
    });

    it('creates major7 and minor7 chords', () => {
      expect(Chord.major7('C').intervals).toEqual([0, 4, 7, 11]);
      expect(Chord.minor7('A').intervals).toEqual([0, 3, 7, 10]);
    });
  });
