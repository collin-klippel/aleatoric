...
describe('Scale', () => {
  ...
  describe('factory methods', () => {
    it('static factories match Scale.create for C root', () => {
      const root = 'C' as const;
      const cases: { factory: () => Scale; type: string }[] = [
        { factory: () => Scale.major(root), type: 'major' },
        { factory: () => Scale.minor(root), type: 'minor' },
        { factory: () => Scale.harmonicMinor(root), type: 'harmonicMinor' },
        { factory: () => Scale.melodicMinor(root), type: 'melodicMinor' },
        { factory: () => Scale.pentatonic(root), type: 'pentatonic' },
        { factory: () => Scale.minorPentatonic(root), type: 'minorPentatonic' },
        { factory: () => Scale.blues(root), type: 'blues' },
        { factory: () => Scale.chromatic(root), type: 'chromatic' },
        { factory: () => Scale.wholeTone(root), type: 'wholeTone' },
        { factory: () => Scale.octatonic(root), type: 'octatonic' },
        { factory: () => Scale.dorian(root), type: 'dorian' },
        { factory: () => Scale.phrygian(root), type: 'phrygian' },
        { factory: () => Scale.lydian(root), type: 'lydian' },
        { factory: () => Scale.mixolydian(root), type: 'mixolydian' },
        { factory: () => Scale.aeolian(root), type: 'aeolian' },
        { factory: () => Scale.locrian(root), type: 'locrian' },
      ];
      for (const { factory, type } of cases) {
        const fromFactory = factory();
        const fromCreate = Scale.create(root, type);
        expect(fromFactory.intervals).toEqual(fromCreate.intervals);
        expect(fromFactory.name).toBe(fromCreate.name);
        expect(fromFactory.root).toBe(fromCreate.root);
      }
    });
