...
describe('rhythm utilities', () => {
  ...
  describe('isDurationName', () => {
    it('returns true for known duration keys', () => {
      expect(isDurationName('quarter')).toBe(true);
      expect(isDurationName('half')).toBe(true);
    });

    it('returns false for unknown strings', () => {
      expect(isDurationName('foo')).toBe(false);
      expect(isDurationName('')).toBe(false);
    });
  });
});
