...
describe('generateRandomPitches', () => {
  ...
  it('throws when no pitches exist in range for the given scale', () => {
    expect(() =>
    ...
    ).toThrow('No pitches available');
  });

  it('falls back to uniform index pick for unknown distribution', () => {
    const events = generateRandomPitches({
      count: 5,
      low: 60,
      high: 60,
      distribution: 'not-a-real-distribution' as never,
      rng: new SeededRng(42),
    });
