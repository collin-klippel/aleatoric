...
describe('generateDiceMusic', () => {
  ...
  it('uses default fragment duration when column is empty', () => {
    let roll = 0;
    const rng: RandomSource = {
      next: () => 0.5,
      nextInt(_min: number, _max: number) {
        roll++;
        if (roll % 2 === 1) return 3;
        return 4;
      },
      nextBool: () => true,
      fork() {
    ...
    const noteColumn = createSimpleDiceTable({ 7: [[60]] }, 1, 80)[7][0];
    ...
    expect(events[0].startBeat).toBe(4);
  });
});
