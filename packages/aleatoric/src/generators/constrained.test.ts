...
describe('constraints', () => {
  ...
  describe('NoParallelFifthsConstraint', () => {
    it('breaks consecutive perfect fifth motion', () => {
      const constraint = new NoParallelFifthsConstraint();
      const events = [makeNote(60, 0), makeNote(67, 1), makeNote(74, 2)];
      const result = constraint.apply(events);
      expect(result[2].midi).not.toBe(74);
      expect(result[2].midi).toBe(75);
    });
  });
  ...
  describe('ContourConstraint', () => {
    ...
    it('descending contour adjusts rising segments downward', () => {
      const constraint = new ContourConstraint('descending');
      const events = [makeNote(60, 0), makeNote(72, 1), makeNote(80, 2)];
      const result = constraint.apply(events);
      for (let i = 1; i < result.length; i++) {
        expect(result[i].midi).toBeLessThanOrEqual(result[i - 1].midi);
      }
    });

    it('arch contour rises then falls', () => {
      const constraint = new ContourConstraint('arch');
      const events = [
        makeNote(60, 0),
        makeNote(55, 1),
        makeNote(50, 2),
      ...
      const result = constraint.apply(events);
      expect(result[1].midi).toBeGreaterThanOrEqual(result[0].midi);
      expect(result[3].midi).toBeLessThanOrEqual(result[2].midi);
    });

    it('valley contour falls then rises', () => {
      const constraint = new ContourConstraint('valley');
      const events = [
        makeNote(72, 0),
        makeNote(60, 1),
        makeNote(50, 2),
      ...
      const result = constraint.apply(events);
      expect(result[1].midi).toBeLessThanOrEqual(result[0].midi);
      expect(result[4].midi).toBeGreaterThanOrEqual(result[3].midi);
    });
  });
