...
describe('DefaultRng', () => {
  ...
  it('nextBool respects probability', () => {
    ...
    let lows = 0;
    for (let i = 0; i < 200; i++) {
      if (!rng.nextBool(0.05)) lows++;
    }
    expect(lows).toBeGreaterThan(150);
  });

  it('fork returns a new DefaultRng instance', () => {
    const parent = new DefaultRng();
    const child = parent.fork();
    expect(child).not.toBe(parent);
    expect(child).toBeInstanceOf(DefaultRng);
  });
});
